...
class computed_purchase_order_line(Model):
    ...
    def _store_stock_duration(self, cr, uid, ids, context=None):
        return self._compute_stock_duration(
            cr, uid, ids, 'stock_duration', 0, None, context=None)
    ...
    _columns = {
        ...
        'state': fields.selection(
            _STATE, 'State', required=True, readonly=True,
        ...
        'uom_id': fields.related(
            'product_id', 'uom_id', string="UoM", type='many2one',
            relation='product.uom', readonly='True'),
        'product_code': fields.char('Supplier Product Code',),
        'product_code_inv': fields.function(
            _get_product_information, fnct_inv=_set_product_code,
        ...
        'product_name_inv': fields.function(
            _get_product_information, fnct_inv=_set_product_name,
        ...
        'product_price_inv': fields.function(
            _get_product_information, fnct_inv=_set_product_price,
            type='float', string='Supplier Product Price',
            multi='product_code_name_price',),
        'package_quantity': fields.float('Package quantity'),
        'package_quantity_inv': fields.function(
            _get_product_information, fnct_inv=_set_package_quantity,
            type='float', string='Package quantity',
            multi='product_code_name_price',),
        'weight_net': fields.related(
            'product_id', 'weight_net', string='Net Weight', type='float',
            readonly='True'),
        ...
        'qty_available': fields.function(
            _get_qty_available, type='float', string='Quantity On Hand',
    ...
    def onchange_product_id(
            self, cr, uid, ids, parent_id, product_id, partner_id,
            context=None):
        if not product_id:
            vals = self._defaults
        else:
            psi_obj = self.pool.get('product.supplierinfo')
            pp = self.pool.get('product.product').browse(
            ...
            psi_id = psi_obj.search(cr, uid, [
                ('name', '=', partner_id),
                ('product_id', '=', pp.product_tmpl_id.id)], context=context)
            if psi_id:
    ...
    def unlink_psi(self, cr, uid, ids, context=None):
        ...
        for cpol in self.browse(cr, uid, ids, context=context):
            ...
            psi_ids = psi_obj.search(cr, uid, [
                ('name', '=', partner_id),
                ('product_id', '=', product_tmpl_id)], context=context)
            psi2unlink += psi_ids
